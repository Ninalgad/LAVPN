...
class UNetMultiTask(nn.Module):
    def __init__(
        ...
        self.center_projection = point_wise_feed_forward_network(32, num_filters * 8, num_filters * 8)
    ...
    def forward(self, x: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
        ...
        v = self.center_projection(v)
        v = torch.unsqueeze(torch.unsqueeze(v, -1), -1)
        center = center + v
        ...
        # global max pool
        center_pool = F.max_pool2d(center, kernel_size=center.size()[2:])
        center_pool = torch.squeeze(center_pool, dim=(2, 3))

        return self.final(dec1), center_pool
...
class MTNet(nn.Module):
    def __init__(self, output_classification_dim=1, output_segmentation_dim=1,
                 hidden_dim=128, pretrained=True, freeze_encoder=False):
        super().__init__()

        self.classification_head = point_wise_feed_forward_network(256, output_classification_dim, hidden_dim)
        self.unet = UNetMultiTask(num_classes=hidden_dim, pretrained=pretrained)
        self.hidden_layer = nn.Conv2d(hidden_dim, hidden_dim, 1)
        self.segmentation_head = nn.Conv2d(hidden_dim, output_segmentation_dim, 1)
    ...
    def forward(self, x, v):
        ...
        c = torch.concat([c, v], dim=-1)
        c = self.classification_head(c)
