...
class UNet16(nn.Module):
    def __init__(
        ...
        self.decoder_layers = [self.dec1, self.dec2, self.dec3, self.dec4, self.dec5, self.final]
...
class EfficientNetV2M(nn.Module):
    def __init__(
        ...
        self.encoder = torchvision.models.efficientnet_v2_m(weights=weights).features
        self.conv1 = self.encoder[:2]
        self.conv2 = self.encoder[2]
        self.conv3 = self.encoder[3]
        self.conv4 = self.encoder[4]
        self.conv5 = self.encoder[5:]

        self.pool = nn.MaxPool2d(2, 2)

        self.center = DecoderBlockV2(
            1280, num_filters * 8 * 2, num_filters * 8, is_deconv
        )

        self.dec5 = DecoderBlockV2(
            1280 + num_filters * 8, num_filters * 8 * 2, num_filters * 8, is_deconv
        )
        self.dec4 = DecoderBlockV2(
            160 + num_filters * 8, num_filters * 8 * 2, num_filters * 8, is_deconv
        )
        self.dec3 = DecoderBlockV2(
            80 + num_filters * 8, num_filters * 4 * 2, num_filters * 2, is_deconv
        )
        self.dec2 = DecoderBlockV2(
            48 + num_filters * 2, num_filters * 2 * 2, num_filters, is_deconv
        )
        self.dec1 = DecoderBlockV2(24 + num_filters, num_filters*2, num_filters)
        self.final = nn.Conv2d(num_filters, num_classes, kernel_size=1)
